...
const MERCHANT_NUMBER = process.env.MOBILE_AIRTEL_MERCHANT_NUMBER || '54321';
const SANDBOX = (process.env.MOBILE_AIRTEL_SANDBOX || 'true') === 'true';
const API_URL = process.env.MOBILE_AIRTEL_API_URL || '';
const API_KEY = process.env.MOBILE_AIRTEL_API_KEY || '';
const CALLBACK_SECRET = process.env.MOBILE_AIRTEL_CALLBACK_SECRET || '';

async function callProviderApi(body: any) {
  if (!API_URL) throw new Error('No Airtel API URL configured');
...
export async function initiatePayment(order: any, phone_number: string) {
  ...
  if (API_URL) {
    try {
      ...
      console.warn('Airtel API call failed, falling back to sandbox instructions', err?.message || err);
    }
  }

  const instructions = SANDBOX
    ? `SANDBOX: Airtel Money: send TZS ${amount.toLocaleString()} from ${phone_number} to ${MERCHANT_NUMBER} with ref ${provider_tx_id}`
    : `Airtel Money: send TZS ${amount.toLocaleString()} from ${phone_number} to ${MERCHANT_NUMBER} with ref ${provider_tx_id}`;
...
export async function handleCallback(payload: any, headers?: any) {
  try {
    if (CALLBACK_SECRET && headers) {
      const signature = headers['x-provider-signature'] || headers['x-airtel-signature'] || headers['x-signature'];
      if (signature) {
    ...
    console.warn('Airtel callback signature verification failed', err?.message || err);
  }
