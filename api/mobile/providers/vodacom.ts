...
const API_URL = process.env.MOBILE_VODACOM_API_URL || '';
const API_KEY = process.env.MOBILE_VODACOM_API_KEY || '';
const CALLBACK_SECRET = process.env.MOBILE_VODACOM_CALLBACK_SECRET || '';

async function callProviderApi(body: any) {
  if (!API_URL) throw new Error('No Vodacom API URL configured');
...
export async function initiatePayment(order: any, phone_number: string) {
  ...
  if (API_URL) {
    try {
      ...
      console.warn('Vodacom API call failed, falling back to sandbox instructions', err?.message || err);
    }
  }
...
export async function handleCallback(payload: any, headers?: any) {
  try {
    if (CALLBACK_SECRET && headers) {
      const signature = headers['x-provider-signature'] || headers['x-vodacom-signature'] || headers['x-signature'];
      if (signature) {
    ...
    console.warn('Vodacom callback signature verification failed', err?.message || err);
  }
